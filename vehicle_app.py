...
client = MongoClient(f"mongodb+srv://{config.username}:{config.password}@cluster1.ppbek.mongodb.net/?retryWrites=true&w=majority")
...
US_Registered_Vehicles = pd.DataFrame(Registered_Vehicles, columns =['year', 'state', 'auto', 'bus', 'truck', 'motorcycle']).astype({'year': 'int', 'state': 'object', 'auto': 'float64', 'bus': 'float64', 'truck': 'float64', 'motorcycle': 'float64'})
...
US_Registered_Vehicles_Year = US_Registered_Vehicles.groupby(["year"]).agg({"auto":'sum',"truck":'sum', "bus":'sum',"motorcycle":'sum'})
